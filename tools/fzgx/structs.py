...
def tu_header(p: Project, module: str, tu: str, min_refs: int = 2) -> str:
    ...
    shared = (Path(__file__).resolve().parent.parent.parent / "include" / "rel" / module / "globals.h")
    ...
    out = [f"#ifndef {guard}", f"#define {guard}", "", '#include "types.h"', f'#include "rel/{module}/globals.h"', "",
...
def write_header(p, module: str, tu: Optional[str] = None, min_refs: int = 20) -> str:
    ...
    out = Path("include") / "rel" / module / (f"{tu.rsplit('.', 1)[0]}.h" if tu else "globals.h")
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(text)
    chk = Path(".fzgx") / "header_selfcheck.c"
    chk.write_text(selfcheck(text, f"rel/{module}/{out.name}"))
